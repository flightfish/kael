...
class YddCore
{
	...
	*method=GET请求示例
	*/
    public static function doGet(string $path, array $queryArr = []) {
		//域名后、query前的部分
        $request = new HttpRequest(self::HOST, $path, HttpMethod::GET, \Yii::$app->params['ydd_appkey'], \Yii::$app->params['ydd_appsecret']);
        ...
        //如果是调用测试环境请设置
        \Yii::$app->params['ydd_test'] && $request->setHeader(SystemHeader::X_CA_STAG, "TEST");
	...
	*/
	private function doPostForm(string $path,array $data) {
		//域名后、query前的部分
		$request = new HttpRequest(self::HOST, $path, HttpMethod::POST, \Yii::$app->params['ydd_appkey'], \Yii::$app->params['ydd_appsecret']);
	...
	*/
	public static function doPostJson($path,$data) {
		//域名后、query前的部分
		$request = new HttpRequest(self::HOST, $path, HttpMethod::POST, \Yii::$app->params['ydd_appkey'], \Yii::$app->params['ydd_appsecret']);
		//传入内容是json格式的字符串
...
//		$bodyContent = "{\"inputs\": [{\"image\": {\"dataType\": 50,\"dataValue\": \"base64_image_string(此行)\"},\"configure\": {\"dataType\": 50,\"dataValue\": \"{\"side\":\"face(#此行此行)\"}\"}}]}";
	...
	*/
    private function doDelete() {
		//域名后、query前的部分
		$path = "/delete";
		$request = new HttpRequest(self::HOST, $path, HttpMethod::DELETE, \Yii::$app->params['ydd_appkey'], \Yii::$app->params['ydd_appsecret']);
	...
	*/
    private function doHead() {
		//域名后、query前的部分
		$path = "/head";
		$request = new HttpRequest(self::HOST, $path, HttpMethod::HEAD, \Yii::$app->params['ydd_appkey'], \Yii::$app->params['ydd_appsecret']);
