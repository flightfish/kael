...
<body>
<form>
    <input name="name" type="text" placeholder="输入名称搜索">
    <input type="submit" value="搜索">
</form>
<div id="file-container">
    选择图片搜索：<input style="display: inline" id="file" type="file">
</div>
<div>图片名称：<?php if(isset($image)) {echo $image['resource_name'];}?></div>
<div>图片：<img src="<?php if(isset($image)) {echo $image['resource_url'];}?>" style="max-height: 100px;"></div>
<table class="table">
    <tr>
        <td>题目id</td>
        <td>题目</td>
...
//         uptoken : '', //若未指定uptoken_url,则必须指定 uptoken ,uptoken由其他程序生成
        ...
        domain: 'http://qiniu.com',   //bucket 域名，下载资源时用到，**必需**
        get_new_uptoken: true,  //设置上传文件的时候是否每次都重新获取新的token
        container: 'file-container',           //上传区域DOM ID，默认是browser_button的父元素，
        max_file_size: '4mb',           //最大文件体积限制
        flash_swf_url: 'js/plupload/Moxie.swf',  //引入flash,相对路径
        max_retries: 3,                   //上传失败最大重试次数
        dragdrop: false,                   //开启可拖曳上传
        drop_element: 'file-container',        //拖曳上传区域元素的ID，拖曳文件或文件夹后可触发上传
        ...
        init: {
            ...
            'FileUploaded': function(up, file, info) {
                ...
                var etag = res.hash;
                window.location.href = "/admin/resource/question-by-image?etag=" + etag;
            },
